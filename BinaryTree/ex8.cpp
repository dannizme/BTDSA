...
void inOrderTraversal(Node *root, std::ofstream &outFile)
{
    if (root == nullptr)
    {
        return;
...
int main()
{
    ...
    std::ofstream outFile("data_ex8.out", std::ios::binary);
    if (!outFile)
    {
    ...
    outFile.close();
    std::cout << "Cây nhị phân đã được xuất ra tập tin data_ex8.out" << std::endl;
